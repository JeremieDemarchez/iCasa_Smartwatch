...
public class MeasureSender implements ISender {
	...
	private static final Logger logger = LoggerFactory.getLogger(MeasureSender.class);

	public boolean send(Data data) {
		if (data != null) {
			Measure measure = (Measure) data.getContent();
			String sensorId = measure.getDeviceId();
			if (sensorId.equals("X10.A1")
					|| sensorId.equals("uuid:7cec7632-1dd2-11b2-9de3-48e5e124c101"))
				return false;

			Date date = new Date(measure.getTimestamp());
			try {
				eventService.processEventData(measure.getLocalisation(),
						measure.getPatientId(), "location", date,
						measure.getRealibility(), measure.getLocalisation());
				logger.info("Adapter Web Service --> message processed");
