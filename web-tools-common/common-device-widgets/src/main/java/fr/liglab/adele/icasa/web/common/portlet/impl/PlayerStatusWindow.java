 ...
 */
package fr.liglab.adele.icasa.web.common.portlet.impl;
...
public class PlayerStatusWindow extends DeviceStatusWindow {
   ...
   private static final long serialVersionUID = 7214188369352385316L;
	...
	public PlayerStatusWindow(BaseHouseApplication parent, String deviceSerialNumber) {
	...
	protected void updateInfo(final ServiceReference reference, final GenericDevice device) {
		...
		if (device instanceof AudioSource) {
			AudioSource audioSrc = (AudioSource) device;
			layoutGrid.add(new Label("Playing: "));
			
			String playing = "YES";
			if (!audioSrc.isPlaying())
				playing = "NO";
			Label locationLabel = new Label(playing + "");
			if (audioSrc.isPlaying())
