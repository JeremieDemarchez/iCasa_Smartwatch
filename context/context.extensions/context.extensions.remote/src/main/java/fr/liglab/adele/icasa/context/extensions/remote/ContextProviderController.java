...
public class ContextProviderController extends DefaultController {
    ...
    @Requires(specification = CreatorHandlerIntrospection.class,optional = true)
    List<CreatorHandlerIntrospection> creatorHandlers;

    List<CreatorHandlerIntrospection> fake = new ArrayList<CreatorHandlerIntrospection>();
    ...
    public Result getContextProviders(){
        ...
        /**    for (CreatorHandlerIntrospection creatorHandler : creatorHandlers){
            result.put(creatorHandler.getAttachedComponentInstanceName(),true);
        }**/
       for (CreatorHandlerIntrospection creatorHandler : fake){
            result.put(creatorHandler.getAttachedComponentInstanceName(),true);
        }
        return ok(result);
    }
