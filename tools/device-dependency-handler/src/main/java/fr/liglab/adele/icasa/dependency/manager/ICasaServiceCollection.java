...
/**
 * Maintains a service object collection. This collection wrap the temporal dependency to be accessible from a
 * {@link Collection}, that can be passed to helper objects (Collaborators).
 * 
 * 
 * The {@link Collection#iterator()} method returns an {@link Iterator} iterating on a cached copy of available service
 * objects. In the case that there are no available services, the policies act as follows:
 * <ul>
 * <li>'null' returns a null iterator</li>
 * <li>'nullable' and default-implementation returns an iterator iterating on one object (the nullable or
...
public class ICasaServiceCollection implements Collection, List, Set {
	 ...
	 */
	public ICasaServiceCollection(DeviceDependency dep) {
		m_dependency = dep;
	}
	...
	/**
	 * Checks if the wrapped dependency has always access to the given service object.The method allows knowing if the
	 * provider returning the service object has left.
	 * 
	 * @param o the service object
	 * @return <code>true</code> if the object is still available, <code>false</code> otherwise.
	...
	/**
	 * Checks if the wrapped dependencies has always access to the given service objects.The method allows knowing if
	 * providers returning the service objects have left.
	 * 
	 * @param c the set of service object
	 * @return <code>true</code> if the objects are still available, <code>false</code> otherwise.
	 * @see java.util.Collection#contains(java.lang.Object)
	 */
	 ...
	 * 
	 * @return <code>true</code> if one provider or more satisfying the dependency are available. Otherwise, returns
	 *         <code>false</code>
	...
	/**
	 * Gets an iterator on the current list of available service objects. The returned iterator iterates on a cached copy
	...
	public Iterator iterator() {
		...
		return new ServiceIterator(obj, m_dependency); // Create the service iterator with the service reference list.
	}
	...
	/**
	 * Returns an array containing available service objects. This method executed on timeout policies if no matching
	...
	public Object get(int index) {
		List list = (List) m_dependency.getService();
		Object svc = list.get(index);
		ICasaAggregateProxyFactory proxyFactory = new ICasaAggregateProxyFactory(m_dependency, svc);
		Object proxiedObject = proxyFactory.getProxy(m_dependency.getSpecification());
	 ...
	 * 
	 * @param o the object
	 * @return the index of the object of <code>-1</code> if not found.
	...
	/**
	 * Gets a list iterator on the current list of available service objects. The returned iterator iterates on a cached
	 * copy of the service objects.
	 * 
	 * @return a iterator giving access to service objects.
	...
	private static final class ServiceIterator implements ListIterator {
		 ...
		 */
		private ServiceIterator(List list, DeviceDependency dependency) {
			m_iterator = list.listIterator();
			m_dependency = dependency;
		}

		/**
		 * Returns <code>true</code> if the iteration has more service objects.
		 * 
		 ...
		 */
		public Object next() {
			return getProxy(m_iterator.next());
		}
		 ...
		 * 
		 * @return true if there is an element before the current one.
		...
		public Object previous() {
			return getProxy(m_iterator.previous());
		}
		...
		private Object getProxy(Object svc) {
			ICasaAggregateProxyFactory proxyFactory = new ICasaAggregateProxyFactory(m_dependency, svc);
			Object proxiedObject = proxyFactory.getProxy(m_dependency.getSpecification());
			return proxiedObject;
		}
