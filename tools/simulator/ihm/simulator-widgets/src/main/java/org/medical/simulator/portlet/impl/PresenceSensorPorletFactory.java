...
public class PresenceSensorPorletFactory implements PortletFactory {
	...
	public boolean typeIsSupported(GenericDevice device) {
		if (device instanceof SetTopBox)
			return false; // TODO find a better way to be sure that this factory
							// does not manage settopbox
	...
	public ResourceImageReference getImageForDevice(GenericDevice device) {
		if (device instanceof GenericDevice) {
			...
			if (medicalGenericDevice.getState().equals("deactivated")) {
				return new ResourceImageReference("/Device-Deactivated.png");
			} else if (medicalGenericDevice.getState().equals("activated")) {
				if (medicalGenericDevice.getFault().equals("yes"))
					return new ResourceImageReference("/Device-Fault.png");

				if (medicalGenericDevice instanceof PresenceSensor) {
					if (((PresenceSensor) medicalGenericDevice)
							.getSensedPresence())
						return new ResourceImageReference(
								"/Device-Activated-Event.png");
				}

				return new ResourceImageReference("/Device-Activated.png");
			}
		}
